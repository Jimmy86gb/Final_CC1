...
public class ClientsView {
    ...
    public void showAddClientDialog(String prefillId) {
        ...
        while(cIt.hasNext()) typeBox.getItems().add(cIt.Next());
        ...
        dialog.showAndWait().ifPresent(res -> {
            if (res == ButtonType.OK) appController.registerClient(idField.getText(), nameField.getText(), typeBox.getValue(), contactField.getText());
        });
    }
    ...
    private void showEditClientDialog(ClientDTO client) {
        ...
        grid.add(new Label("ID/Cedula:"), 0, 0); grid.add(new Label(client.getId()), 1, 0);
        grid.add(new Label("Nombre:"), 0, 1);    grid.add(nameField, 1, 1);
        grid.add(new Label("Tipo:"), 0, 2);      grid.add(typeBox, 1, 2);
        grid.add(new Label("Contacto:"), 0, 3);  grid.add(contactField, 1, 3);
        ...
        dialog.showAndWait().ifPresent(res -> {
            if (res == ButtonType.OK) appController.updateClient(client.getId(), nameField.getText(), typeBox.getValue(), contactField.getText());
        });
    }
