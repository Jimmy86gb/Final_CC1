...
/**
 * Sub-controlador core encargado de la gestion operativa de siniestros y
 * despacho. Actua como el motor del modulo de "Despacho y Control de
 * Emergencias", coordinando el ciclo de vida de los reportes a traves de sus
 * diferentes estados (Pendiente, En Progreso, Por Confirmar) y gestionando la
 * asignacion de recursos (Tecnicos, Unidades y Kits). * @author Jimmy86gb
 */
public class RequestsController {
	...
	/**
	 * Constructor de la clase RequestsController. Ensambla el controlador
	 * inicializando su vista interactiva y todos los casos de uso requeridos para
	 * la logica de colas, pilas y asignaciones cruzadas. * @param appController
	 * Controlador principal para delegacion de alertas y navegacion.
	 * 
	 * @param role       Rol del usuario actual para ajustar los permisos en el
	 *                   Kanban.
	...
	public RequestsController(AppController appController, String role, ReportRepository reportRepo,
		...
		this.undoReportResourcesUseCase = new UndoReportResourcesUseCase(reportRepo);
	...
	/**
	 * Obtiene la vista asociada a este sub-controlador. * @return Objeto
	 * RequestsView con la interfaz del tablero Kanban.
	 */
	...
	/**
	 * Refresca completamente el tablero de control de emergencias. Limpia los
	 * paneles visuales y reconstruye las colas y pilas iterando sobre las
	 * estructuras de datos, garantizando que no se generen duplicados
	 * ("fantasmas").
	 */
	...
	/**
	 * Inicia el flujo de creacion de un nuevo reporte. Verifica previamente la
	 * existencia del cliente; si no existe, delega el flujo hacia el registro de
	 * clientes, de lo contrario abre el formulario de siniestro. * @param clientId
	 * Cedula o NIT ingresado en el dialogo inicial.
	 */
	...
	/**
	 * Ejecuta el registro formal de la emergencia, insertandola en la cola
	 * prioritaria. * @param cliId Identificacion del cliente afectado.
	 * 
	...
	public void submitReportCreation(String cliId, String desc, String type, String prio, String zone) {
		appController.handleResponse(registerReportUseCase.execute(cliId, desc, type, prio, zone),
				appController::refreshAllViews);
	}

	/**
	 * Asigna los recursos operativos a una emergencia y la traslada a la fase de
	 * ejecucion. * @param techId UUID del tecnico seleccionado.
	 * 
	...
	public void assignReportResources(String techId, String unitId, String kitId) {
		appController.handleResponse(assignResourcesReportUseCase.execute(techId, unitId, kitId),
				appController::refreshAllViews);
	}

	/**
	 * Marca una atencion en terreno como finalizada, moviendola a la pila de
	 * confirmacion. * @param ticketId UUID del reporte.
	 */
	...
	/**
	 * Aprueba la resolucion del siniestro actual en el tope de la pila de
	 * confirmacion, liberando los recursos asociados (tecnico, unidad, kit).
	 */
	...
	/**
	 * Rechaza la accion pendiente en el tope de la pila de confirmacion y devuelve
	 * el siniestro a su estado anterior.
	 */
	 ...
	 * el siniestro a su estado anterior.
	 */
	public void undoResourses() {
		appController.handleResponse(undoReportResourcesUseCase.execute(), appController::refreshAllViews);
	}

	/**
	 * Solicita la cancelacion de un siniestro que aun se encuentra en la cola de
	 * espera. * @param ticketId UUID del reporte a cancelar.
	 */
	public void cancelReport(String ticketId) {
		appController.handleResponse(requestReportCancellationUseCase.execute(ticketId),
				appController::refreshAllViews);
	}

	/**
	 * Extrae visualmente (sin retirar) la emergencia mas urgente de la cola de
	 * espera. * @return Objeto ReportDTO con los datos del siguiente siniestro a
	 * atender.
	 */
	...
	/**
	 * Obtiene una lista filtrada de tecnicos disponibles y capacitados para una
	 * zona y especialidad. Aplica normalizacion de cadenas para asegurar la
	 * busqueda. * @param zoneName Nombre de la zona del incidente.
	 * 
	...
	public SimpleList<EntityItem> getSuggestedTechnicians(String zoneName, String specialtyName) {
		...
		if (dtos.getSize() == 0) {
			dtos = getAvailableTechnicianUseCase.execute(zoneName, specialtyName);
		}
	...
	public SimpleList<EntityItem> getSuggestedUnits(String zoneName) {
		...
		if (dtos.getSize() == 0) {
			dtos = getAvailableUnitsUseCase.execute(zoneName);
		}
	...
	public SimpleList<EntityItem> getSuggestedKits(String problemType) {
		...
		if (dtos.getSize() == 0) {
			dtos = getAvailableKitsUseCase.execute(problemType);
		}
