...
public class AppController {
    ...
    // FILTROS NORMALIZADOS (Evita Combobox Vacíos)
    ...
    public SimpleList<EntityItem> getSuggestedUnits(String zoneName) {
        SimpleList<EntityItem> list = new SimpleList<>();
        String zNorm = zoneName != null ? zoneName.replace(" ", "") : "";
        var dtos = getAvailableUnitsByZoneUseCase.execute(zNorm);
    ...
    public SimpleList<EntityItem> getSuggestedKits(String problemType) {
        ...
        var dtos = getAvailableKitsByTypeUseCase.execute(pNorm);
    ...
    // --- CRUD ---
    public void registerClient(String id, String name, String type, String contact) { handleResponse(registerClientUseCase.ResponseDTO(id, name, type, contact), this::refreshAllViews); }
    public void updateClient(String id, String name, String type, String contact) { handleResponse(updateClientUseCase.execute(id, name, type, contact), this::refreshAllViews); }
    public void registerTechnician(String name, String specialty, String zone) { handleResponse(registerTechnicianUseCase.execute(name, specialty, zone), this::refreshAllViews); }
    public void updateTechnician(String id, String name, String specialty, String zone, String status) { handleResponse(updateTechnicianUseCase.execute(id, name, specialty, zone, status), this::refreshAllViews); }
    public void registerUnit(String type, String zone, int quantity) { handleResponse(registerServiceUnitUseCase.execute(type, zone, quantity), this::refreshAllViews); }
    public void updateServiceUnit(String id, String type, String status, String zone) { handleResponse(updateServiceUnitUseCase.execute(id, type, status, zone), this::refreshAllViews); }
    public void approveUnitStatus() { handleResponse(approveUnitStatusUseCase.execute(), this::refreshAllViews); }
    public void rejectUnitStatus() { handleResponse(rejectUnitStatusUseCase.execute(), this::refreshAllViews); }
    public void registerKit(String type, int quantity) { handleResponse(registerKitUseCase.execute(type, quantity), this::refreshAllViews); }
    public void updateKitToMaintenance(String id, String type) { handleResponse(updateKitUseCase.execute(id, type, "Mantenimiento"), this::refreshAllViews); }
    public void returnKitToService() { handleResponse(returnKitToServiceUseCase.execute(), this::refreshAllViews); }
    public void retireKitFromMaintenance() { handleResponse(retireKitFromMaintenanceUseCase.execute(), this::refreshAllViews); }
    ...
    public void finishReport(String ticketId) { handleResponse(finishReportInFieldUseCase.execute(ticketId), this::refreshAllViews); }
    public void approveReport() { handleResponse(approveReportActionUseCase.execute(), this::refreshAllViews); }
    public void rejectReport() { handleResponse(rejectReportActionUseCase.execute(), this::refreshAllViews); }
    public void cancelReport(String ticketId) { handleResponse(requestReportCancellationUseCase.execute(ticketId), this::refreshAllViews); }
    
    public ReportDTO getNextPendingReport() { return getNextPendingReportUseCase.execute(); }
    public void exportDailyReport() { handleResponse(generateDailyCSVUseCase.execute(""), () -> logAction("REPORTE CSV: Generado con éxito.")); }
    ...
    private void refreshUnitsView() {
        ...
        // Unidades Por Confirmar (Pila Admin)
    ...
    private void injectSeedData() {
        ...
        // Quitamos espacios por seguridad del Factory
