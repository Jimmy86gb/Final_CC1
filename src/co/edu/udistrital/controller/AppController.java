...
public class AppController {
	...
	public AppController() {
		...
		this.getSortedAndDFilteredTechniciansUseCase = new GetSortedAndDFilteredTechniciansUseCase(
		...
		this.getSortedAndFilteredServiceUnitsUseCase = new GetSortedAndFilteredServiceUnitsUseCase(
		...
		this.saveSystemDataUseCase = new SaveSystemDataUseCase(profileRepository, clientRepository,
				technicianRepository, serviceUnitRepository, kitRepository, reportRepository);
		this.loadSystemDataUseCase = new LoadSystemDataUseCase(profileRepository, clientRepository,
				technicianRepository, serviceUnitRepository, kitRepository, reportRepository);
	...
	public void performUndo() {
		...
		if (res.isSuccess()) {
			logAction("REVERSIÓN EJECUTADA: " + res.getMessage());
		}
	...
	public void submitReportCreation(String cliId, String desc, String type, String prio, String zone) {
		...
		if (res.isSuccess()) {
			logAction("EMERGENCIA REGISTRADA: Cliente " + cliId);
		}
		handleResponse(res, this::refreshAllViews);
	}
	...
	public void assignReportResources(String techId, String unitId, String kitId) {
		...
		if (res.isSuccess()) {
			logAction("ASIGNACIÓN: TKT vinculado a Tec: " + techId + " / Unidad: " + unitId);
		}
		handleResponse(res, this::refreshAllViews);
	}
	...
	public void finishReport(String ticketId) {
		...
		if (res.isSuccess()) {
			logAction("FINALIZADO EN CAMPO: Siniestro " + ticketId);
		}
		handleResponse(res, this::refreshAllViews);
	}
	...
	public void approveReport() {
		...
		if (res.isSuccess()) {
			logAction("APROBADO POR ADMIN: Cierre exitoso");
		}
		handleResponse(res, this::refreshAllViews);
	}
	...
	public void rejectReport() {
		...
		if (res.isSuccess()) {
			logAction("RECHAZADO: Siniestro retornado a progreso");
		}
		handleResponse(res, this::refreshAllViews);
	}
	...
	private void refreshDashboardView() {
		if (dashboardView != null) {
			...
			while (uit.hasNext()) {
				if (!uit.Next().getStatus().equals("Mantenimiento")) {
					activeUnits++;
				}
			}
	...
	private void refreshUnitsView() {
		...
		while (uIt.hasNext()) {
			...
			unitsView.addUnit(u.getId().toString(), u.getType(), u.getStatus(), u.getZone(),
					u.getStatus().equals("Disponible"));
		}
	}
	...
	private void refreshTechniciansView() {
		...
		while (tIt.hasNext()) {
			...
			techniciansView.addTechnician(t.getId().toString(), t.getName(), t.getSpecialty(), t.getStatus(),
					t.getZone(), t.getStatus().equals("Disponible"));
		}
	}
	...
	/**
	 * Inyecta datos predefinidos de prueba para la sustentación. Solo se ejecuta si
	 * el sistema no encuentra archivos de guardado previos.
	 */
	private void injectSeedData() {
		...
		// 2. Crear Técnicos (Usamos los nombres técnicos sin espacios como los espera
		// el Factory)
		...
		registerReportUseCase.execute("102030", "Camión bloqueado, falla de motor principal.", "MecanicoGeneral",
				"Alta", "Suba");
		registerReportUseCase.execute("405060", "Choque múltiple en la avenida, requiere traslado.", "OperadordeGrua",
				"Alta", "Kennedy");
		registerReportUseCase.execute("708090", "Vehículo no enciende, posible batería muerta.", "ElectricoAutomotriz",
				"Media", "Chapinero");
		registerReportUseCase.execute("102030", "Llaves olvidadas dentro del vehículo.", "CerrajerodeVehiculos", "Baja",
				"Usaquen");

		// 6. Guardar automáticamente en disco para que no se vuelva a inyectar la
		// próxima vez
